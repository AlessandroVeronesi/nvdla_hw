...
# Output directory
export PROJDIR="${OUTDIR}/${PROJECT}"
if [ ! -d "$PROJDIR" ] ; then
    echo "[ERROR]: Please provide a valid project."
    usage
fi
...
#if [ -z "$simoptions" ] && [ -z "$SIM_OPTIONS" ] ; then
#    echo "[ERROR]: SIM_OPTIONS cannot be empty. Aborting"
#    exit
#elif [ -z "$simoptions" ] ; then
#    simoptions=$SIM_OPTIONS
#fi

#if [ ! -z "${build}" ] ; then
#    if [[ "${TOP_NAMES}" =~ ( |\') ]]; then
#        export build="nvdla_syn_$timestamp"
#    else
#        export build="${TOP_NAMES}_syn_$timestamp"
#    fi
#fi

export BUILD_NAME="$PROJDIR/syn/$build"
...
dataprep()
{
    ...
    echo "" > $BUILD_NAME/verif/dut.f
    for option in $simoptions
    do
        echo  "$option" >> $BUILD_NAME/verif/dut.f
    done
}
